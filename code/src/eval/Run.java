...
public class Run {
	...
	static String DOM_MNIST_="M";
	
	// directory
	static String expDir_="/cs/grad/ronent/Desktop/link_home/repos/spn-release";
	...
	static String mnistDataDir_=expDir_+"/data/mnist";
	static String mnistRstDir_=expDir_+"/results/mnist/completions";
	static String mnistMdlDir_=expDir_+"/results/mnist/models";
	...
	static void proc(String[] args) throws Exception {	
		...
		else if (Parameter.domain_.equals(DOM_MNIST_)) {
			runMNIST();
		}		
		MPI.Finalize();
	}
	...
	static void runMNIST() throws Exception {
		MyMPI.setConstantsForImgs();		
		Dataset data=new Dataset();
		data.loadMNIST();
		...
		SPN dspn=l.getDSPN();
		if (MyMPI.myOffset_==0) {
			dspn.saveDSPN(mnistMdlDir_+"/mnist");
		}
		
		// complete
		ImageCompletion.completeLeft(dspn, data.getTest(), "mnist", mnistRstDir_);
		ImageCompletion.completeBottom(dspn, data.getTest(), "mnist", mnistRstDir_);
	}
}
