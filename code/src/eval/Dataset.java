...
public class Dataset {
	...
	static String mnistRawFileName_=expDir_+"/data/mnist/mnist_train.data";
	...
	static void setInstance(int[][] buf, Instance inst) {	
		...
		for (int i=0; i<Parameter.inputDim1_; i++)
			for (int j=0; j<Parameter.inputDim2_; j++) {
				// adding normalization to [-0.5,0.5] range (for MNIST experiment)
				inst.vals_[i][j]= ( ( (double) buf[i][j] ) / 255) - 0.5; 
	...
	public void loadMNIST() throws Exception {
		Set<Integer> tis=genTestIdx(50000,Parameter.maxTestSize_);
		BufferedReader in;
		in = new BufferedReader(new FileReader(mnistRawFileName_));
		double[][] digits=new double[784][50000];
		String s;
		String[] ts;
		int idx=0;		
		while ((s=in.readLine())!=null) {
			...
			ts=s.split("  ");
			for (int i=0; i<50000; i++) digits[idx][i]=Double.parseDouble(ts[i]);
			idx++;
		}
		in.close();
		...
		for (int pi=0; pi<50000; pi++) {
			Instance inst=readMNISTInstance(digits, pi);
			if (tis.contains(pi)) test_.add(inst);
			else train_.add(inst);
		}
	}
	
	static Instance readMNISTInstance(double[][] digits, int pi) {
		Instance inst=new Instance();
		for (int i=0; i<Parameter.inputDim1_; i++)
			for (int j=0; j<Parameter.inputDim2_; j++) {
				int k=i*Parameter.inputDim2_+j;
				tmp_[i][j]=(int)digits[k][pi];
			}
		setInstance(tmp_, inst);
		return inst;
	}	
