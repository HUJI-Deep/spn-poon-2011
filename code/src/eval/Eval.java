...
public class Eval {
	...
	static void evalMNIST() throws Exception {
		int size=Parameter.inputDim1_, padLen=10;
		String mnistDir=expDir_+"/results/mnist";
		double lm=-1; try {lm=cmpMSELeft(mnistDir+"/completions/mnist-left.dat", size, padLen);} catch(Exception e){}
		double bm=-1; try {bm=cmpMSEBottom(mnistDir+"/completions/mnist-btm.dat", size, padLen);} catch(Exception e){}
		
		System.out.println("\n\nMNIST MSE\tLeft="+Math.round(lm)+"\tBottom="+Math.round(bm));
	}
